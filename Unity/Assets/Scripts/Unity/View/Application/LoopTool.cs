...
namespace Unity.View
{
	public class LoopTool : IView
	{
		...
		private void SetInput( DirectoryInfo aDirectoryInfo )
		{
			...
			viewLoopInputlist = new ViewLoopInputlist( aDirectoryInfo, PlayMusic, GetPlayingMusic );
		...
		public void OnGUI()
        {
			...
			GUILayout.BeginArea( new Rect( Screen.width / 2 - 150.0f, lY, 300.0f, 200.0f ) );
			{
				GUILayout.Label( new GUIContent( "", "StylePlayer.ToogleLoop" ), GuiStyleSet.StylePlayer.toggleLoop );
			...
			{
				GUILayout.BeginVertical( GuiStyleSet.StyleGeneral.box );//, GUILayout.Width( Screen.width / 2.0f ) );
				{
