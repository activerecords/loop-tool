...
namespace Unity.View
{
	public class ViewLoopSearch : IView
	{
		...
		{
			for( int i = 0; i < dataLoopInputlist.fileInfoList.Count; i++ )
			{
				if( dataLoopInputlist.isSelectedList[i] == true )
				{
					Debug.Log( "Search:" + dataLoopInputlist.fileInfoList[i] );
					
					LoopSearchExecutor.Execute( dataLoopInputlist.fileInfoList[i], dataLoopPlaylist.directoryInfo.FullName + "/" + dataLoopInputlist.fileInfoList[i].Name, dataLoopInputlist.progressList, i );
