...
namespace Curan.Common.FormalizedData.File.Riff.Sfbk
{
	public class RiffChunkPhdr : RiffChunk
	{
		...
		public void Display( RiffBodyPbag pbagChunk, RiffBodyPgen pgenChunk, RiffBodyPmod pmodChunk, Dictionary<int, List<Instrument>> instrumentListDictionary )
		{
			...
			for( int i = 1; i * 38 < Size; i++ )
			{
				if( i == 1 )
				{
					Logger.LogNormal( "■Preset Headers:" + i );
				}
				...
				for( int j = 0; j < pbagData1.gens; j++ )
				{
					...
					case SFGenerator.StartAddrsOffset:
						...
						Logger.LogNormal( phdrDataArray[i - 1].name + "/Start Addrs Offset:" + instrumentListDictionary[instrument].startAddrsOffset );
						break;

					case SFGenerator.EndAddrsOffset:
						instrumentListDictionary[instrument].endAddrsOffset = generator.GetInt16();
						Logger.LogNormal( phdrDataArray[i - 1].name + "/End Addrs Offset:" + instrumentListDictionary[instrument].endAddrsOffset );
						break;

					case SFGenerator.StartLoopAddrsOffset:
						instrumentListDictionary[instrument].startLoopAddrsOffset = generator.GetInt16();
						Logger.LogNormal( phdrDataArray[i - 1].name + "/Start Loop Addrs Offset:" + instrumentListDictionary[instrument].startLoopAddrsOffset );
						break;

					case SFGenerator.EndLoopAddrsOffset:
						instrumentListDictionary[instrument].endLoopAddrsOffset = generator.GetInt16();
						Logger.LogNormal( phdrDataArray[i - 1].name + "/End Loop Addrs Offset:" + instrumentListDictionary[instrument].endLoopAddrsOffset );
						break;

					case SFGenerator.StartAddrsCoarseOffset:
						instrumentListDictionary[instrument].startAddrsCoarseOffset = generator.GetInt16();
						Logger.LogNormal( phdrDataArray[i - 1].name + "/Start Addrs Coarse Offset:" + instrumentListDictionary[instrument].startAddrsCoarseOffset );
						break;
					...
					case SFGenerator.OverridingRootKey:
						...
						Logger.LogNormal( "Root key:" + instrumentListDictionary[instrument].rootKey );
	...
	public class PhdrData
	{
		...
		public PhdrData( ByteArray aByteArray, List<string> aInformationList )
		{
