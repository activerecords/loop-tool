...
namespace Unity.Function.Graphic
{
    public static class Gui
    {
		...
		public static void BeginArea( Rect aRect )
		{
			position.x = aRect.x;
			position.y = aRect.y;
		}
		...
		public static void Label( String aText, GUIStyle aStyle )
		{
			DrawUiTexture( new Rect( position.x, position.y, aStyle.fixedWidth, aStyle.fixedHeight ), aStyle );
		}
		
		public static void Button( String aText, GUIStyle aStyle )
		{
			float lWidth = aStyle.fixedWidth;
			float lHeight = aStyle.fixedHeight;

			if( aStyle.fixedWidth == 0 )
			{
				lWidth = aStyle.CalcSize( new GUIContent( aText ) ).x;
			}
			
			if( aStyle.fixedHeight == 0 )
			{
				lHeight = aStyle.CalcSize( new GUIContent( aText ) ).y;
			}

			DrawUiTexture( new Rect( position.x, position.y, lWidth, lHeight ), aStyle );
		}
